...
def build_model(num_labels, is_pretrained, is_parallel):
  ...
  if is_parallel:
    ...
    model_features = model.module.fc.in_features # input features
    model.module.fc = nn.Sequential(nn.BatchNorm1d(model_features), nn.ReLU(), nn.Dropout(0.25), nn.Linear(model_features, num_labels))
    # num_labels - output features 
    print('Number of labels (Features) - ', num_labels)
    # check in predicate matrix binary 50 classes(rows) and 85 features(columns)
  else:
...
def train(num_epochs, eval_interval, learning_rate, output_filename, model_name, optimizer_name, batch_size):
  ...
  test_params = {'batch_size': 1, 'shuffle': True, 'num_workers': 3} # shuffling through classes
  ...
  train_loader = data.DataLoader(train_dataset, **train_params) # dividing the data 
  test_loader = data.DataLoader(test_dataset, **test_params)
  criterion = nn.BCELoss() #nn.BCELoss()

  # print train_loader and how ?

  total_steps = len(train_loader) #calculation total number of steps
  #to print the total steps
...
if __name__ == '__main__':
  ...
  train_classes = np.array(np.genfromtxt('/tmp/data/Animals_with_Attributes2/trainclasses.txt', dtype='str'))
  classes = np.array(np.genfromtxt('/tmp/data/Animals_with_Attributes2/classes.txt', dtype='str'))[:,-1]
  predicates = np.array(np.genfromtxt('/tmp/data/Animals_with_Attributes2/predicates.txt', dtype='str'))[:,-1]
  predicate_binary_mat = np.array(np.genfromtxt('/tmp/data/Animals_with_Attributes2/predicate-matrix-binary.txt', dtype='int'))
  predicate_continuous_mat = np.array(np.genfromtxt('/tmp/data/Animals_with_Attributes2/predicate-matrix-continuous.txt', dtype='float'))
